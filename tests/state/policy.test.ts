...
describe("listWorkflowStages", () => {
  ...
  it("deduplicates stages by normalized key in listWorkflowStages", () => {
    // Kills: appendStage guard for duplicate key
    const config = createConfig({
      activeStates: ["Todo", "In Progress", "In Progress"],
      terminalStates: ["Done", "Done"],
    });
    const stages = listWorkflowStages(config);
    const todoCount = stages.filter((s) => s.key === "todo").length;
    const ipCount = stages.filter((s) => s.key === "in progress").length;
    const doneCount = stages.filter((s) => s.key === "done").length;
    expect(todoCount).toBe(1);
    expect(ipCount).toBe(1);
    expect(doneCount).toBe(1);
  });

  it("skips empty stage keys in listWorkflowStages", () => {
    // Kills: appendStage guard for empty key
    const config = createConfig({
      activeStates: ["", "Todo"],
      terminalStates: ["Done"],
    });
    const stages = listWorkflowStages(config);
    expect(stages.every((s) => s.key.length > 0)).toBe(true);
    expect(stages).toHaveLength(2);
  });

  it("skips empty stage names in state machine config", () => {
    const base = createConfig();
...
// -------------------------------------------------------------------------
// Mutation-killing: getStateMachine caching and construction
// -------------------------------------------------------------------------

describe("getStateMachine", () => {
  // Import getStateMachine separately for direct testing
  it("creates machine from tracker states when no stateMachine config", () => {
    const config = createConfig();
  ...
  it("returns cached machine for same stateMachine config object", () => {
    const config = createStateMachineConfig();
    const machine1 = getStateMachine(config);
    const machine2 = getStateMachine(config);
    expect(machine1).toBe(machine2); // same reference = cached
  });

  it("creates machine with stages and transitions from stateMachine config", () => {
    const base = createConfig();
    const config: ServiceConfig = {
      ...base,
      stateMachine: {
...
// -------------------------------------------------------------------------
// Mutation-killing: getStageSets caching
// -------------------------------------------------------------------------

describe("getStageSets caching", () => {
  it("returns cached results for the same stateMachine config object", () => {
    const config = createStateMachineConfig();
    // Call isActiveState twice — second call should use cache
    expect(isActiveState("In Progress", config)).toBe(true);
    expect(isActiveState("In Progress", config)).toBe(true);
    // Also gate
    expect(isGateState("Gate Review", config)).toBe(true);
...
// -------------------------------------------------------------------------
// Mutation-killing: getTrackerStateSets caching
// -------------------------------------------------------------------------

describe("getTrackerStateSets caching", () => {
  it("returns cached results for the same config object", () => {
    const config = createConfig();
    // Call multiple times — exercises the cache path
    expect(isActiveState("Todo", config)).toBe(true);
    expect(isActiveState("Todo", config)).toBe(true);
    expect(isTerminalState("Done", config)).toBe(true);
    expect(isTerminalState("Done", config)).toBe(true);
