...
function buildAuthJson(overrides: Record<string, unknown> = {}): string {
...
describe("refreshAccessToken", () => {
  ...
  it("exchanges refresh_token for a new access_token and rewrites auth.json", async () => {
    const dir = await createTempDir();
    const authPath = path.join(dir, "auth.json");
    await writeFile(authPath, buildAuthJson({ expired: "2020-01-01T00:00:00Z" }), "utf8");
    ...
    expect(parsed.expired).toBeTruthy();
  ...
  it("throws when no refresh_token is present", async () => {
    const dir = await createTempDir();
    const authPath = path.join(dir, "auth.json");
    await writeFile(authPath, buildAuthJson({ refresh_token: null }), "utf8");
  ...
  it("throws when the token endpoint returns an error", async () => {
    const dir = await createTempDir();
    const authPath = path.join(dir, "auth.json");
    await writeFile(authPath, buildAuthJson(), "utf8");
  ...
  it("preserves extra fields from the original auth.json", async () => {
    const dir = await createTempDir();
    const authPath = path.join(dir, "auth.json");
    const authWithExtras = buildAuthJson({ email: "user@example.com", account_id: "acc-123" });
