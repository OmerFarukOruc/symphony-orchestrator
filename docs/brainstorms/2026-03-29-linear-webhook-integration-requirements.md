...
Symphony detects Linear issue changes by polling the GraphQL API every 15 seconds. This creates up to 15s latency before new or transitioned issues are picked up, generates unnecessary API traffic when nothing has changed, and locks the architecture into a request-driven model that cannot react to events in real time. Adding inbound Linear webhooks fixes all three: near-instant detection, demand-driven fetching, and a reusable event ingestion pattern for future sources.

## Event & Data Flow

```
Linear Cloud                        Symphony (public URL)
─────────────                       ────────────────────
  Issue mutated
       │
       ▼
...
**Webhook Receiver**
- R1. Expose a `POST /webhooks/linear` endpoint on Symphony's HTTP server that accepts Linear webhook payloads
...
- Issue state changes in Linear are detected and acted on by Symphony within 2 seconds (p95) when webhooks are healthy, vs. up to 15s today
- Polling API calls to Linear drop by 80%+ during normal webhook operation
- If webhooks stop delivering, Symphony automatically recovers to full polling within one threshold window — no operator intervention, no missed issues
...
- **Out of scope**: Generic webhook dispatch API (Issue #32) — this work focuses on Linear as an event *source*, not Symphony as a webhook *receiver* for arbitrary external triggers. The two should share infrastructure where natural, but #32 has its own scope.
...
- **Out of scope**: Webhook replay/retry from Linear's side — Linear handles its own retry logic. Symphony just needs to be idempotent in processing.
- **Out of scope**: Multi-instance webhook coordination — Symphony is a single-instance process. Webhook registration assumes one instance owns the subscription.
...
- **Broad event subscription over selective**: Subscribe to all issue mutations rather than specific event types. Avoids reconfiguring Linear webhooks as Symphony's needs evolve. The event router inside Symphony decides what to act on.
...
- **Single instance constraint**: Symphony is always a single-instance process. No multi-instance webhook coordination, deduplication, or leader election.

## Dependencies / Assumptions

- Linear API token used by Symphony has webhook create/update permissions for auto-registration (or operator handles manual setup)
- Symphony's HTTP server is reachable at the configured `webhook_url` from the public internet (Cloudflare subdomain + tunnel, cloud deploy, or reverse proxy)
- Linear's webhook payload format and signing mechanism are stable (they use standard HMAC-SHA256)
- Symphony runs as a single instance — no multi-instance coordination needed
...
- [Affects R13-R16][Technical] Webhook stats need new fields on `RuntimeSnapshot` and new event types on `SymphonyEventMap`. Plan should name the integration points.
