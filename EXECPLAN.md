...
- [x] (2026-03-23) Implemented Playwright E2E test suite for the dashboard frontend: added `playwright.config.ts` with smoke and visual projects, `tsconfig.e2e.json` for E2E TypeScript, full mock API layer (`tests/e2e/mocks/api-mock.ts`, `scenario-builder.ts`, 6 data factories), 7 Page Object Models (`base`, `setup`, `app-shell`, `overview`, `command-palette`, `queue`, `issue`, `config`), support utilities (`clock.ts`, `screenshot-css.ts`, `unhandled-api.ts`, `screenshot.css`), 37 smoke tests across 6 spec files (`setup-gate`, `overview`, `command-palette`, `queue-issue`, `issue-runs-logs`, `config-secrets`), and 3 visual regression baselines (`setup`, `overview`, `queue`). Updated `.github/workflows/ci.yml` with sharded `e2e-smoke` (3 shards + merged HTML report), `e2e-visual` jobs, and added E2E to the `build-and-test` gate. Updated `tsconfig.eslint.json` to include `playwright.config.ts`.
